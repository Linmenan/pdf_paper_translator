...
[FIXME]目前经过步骤 1.布局标注 -> 开始提取 -> 自动跳转到 2.任务列表 这时任务列表显示正常的，但是此时返回文献库，再次进入 2.任务列表就无法显示任务列表了，也就是说任务列表页面没有做到离线文件缓存。应支持已拆分好的文献直接进入任务列表并正常显示。
[FIXME]2.任务列表页面有“开始翻译”与“开始翻译所有任务”两个按钮设计重复，应将“开始翻译所有任务删除”。
[FIXME]点击开始翻译后会立即自动跳转到 3.结果预览页面，应该不自动跳转，留在该页面，前端按钮“开始翻译”变为停止翻译，并且与后端翻译的代码进行交互，由后端主动发起页面刷新，每完成一个任务，就更新该任务的翻译结果，总数量统计信息。
