...
3.  **图表标题 (Caption):**
    * 遇到 `[[CAPTION: ...]]` 必须输出为 <caption_block>。
    <caption_block>
      <src>Figure 1. Overview</src>
      <trans>图1. 总览</trans>
    </caption_block>

4.  **参考文献 (Ref):**
    * 遇到 References 后的列表，保留原文，不翻译。
    <ref_block><src>...</src></ref_block>

5.  **锚点 (Asset):**
    <asset_anchor>Figure_P1_1</asset_anchor>
...
class LayoutEditor:
    def __init__(self, doc, initial_data):
        ...
        help_txt = "1-图 2-表 3-式 4-算 | [0-正文范围(黄色)] | 右键删除 | 空格下一页"
        self.help_label = tk.Label(self.control_panel, text=help_txt, 
                                   font=(self.ui_font, 11), bg="#f0f0f0", fg="#555")
    ...
    def load_page(self):
        ...
        type_map = {"Figure": "图", "Table": "表", "Equation": "公式", "Algorithm": "算法", "ContentArea": "正文区域"}
    ...
    def next_page(self):
        ...
        if current_content_rect and next_idx < self.page_count:
            # 检查页面尺寸是否一致，防止错位
            curr_page_size = self.doc[self.current_page].rect
            next_page_size = self.doc[next_idx].rect
            
            if curr_page_size == next_page_size:
                if next_idx not in self.data: self.data[next_idx] = []
                
                # 暴力删除下一页已有的 ContentArea (因为那是自动生成的默认值)
                ...
                print(f"   ℹ️ [继承] 已将 P{self.current_page+1} 的黄色正文框应用到 P{next_idx+1}")
            else:
                print(f"   ⚠️ [跳过] P{next_idx+1} 尺寸不同，停止继承正文框")
        # ----------------------------------
...
def extract_text_and_save_assets_smart(pdf_path: str, raw_text_dir: str, vis_output_root: str) -> tuple[str, str, str, int]:
    ...
    caption_pattern = re.compile(r'^\s*(Figure|Fig\.|Table|Tab\.|Algorithm)\s*(\d+|[IVX]+)', re.IGNORECASE)
        ...
        for b in blocks:
            ...
            if not is_in:
                ...
                if is_header:
                    ...
                    if len(lines) > 1:
                        mixed.append({'bbox': bbox, 'text': "\n".join(lines[1:]), 'is_header': False, 'is_asset': False})
                else:
                    mixed.append({'bbox': bbox, 'text': text, 'is_header': False, 'is_caption': is_caption, 'is_asset': False})
    ...
    for i, c in enumerate(chunks):
        ...
        for _ in range(MAX_RETRIES):
            try:
                ...
                if r.text: 
                    clean = re.sub(r'\(\s*(Part|第)\s*\d+\s*(部分)?\s*\)', '', r.text, flags=re.IGNORECASE)
                    clean = re.sub(r'（\s*第\s*\d+\s*部分\s*）', '', clean)
                    res.append(clean)
                    print(" ✅"); ok = True; break
...
def generate_html_report(llm_result_path: str, paper_vis_dir: str):
    ...
    display_title = raw_name.replace('_', ' ')
    ...
    def replace_raw_tags(match):
        tag, txt = match.group(1), match.group(2)
        if tag == "HEADER": return f"<header_block><src>{txt}</src><trans>{txt}</trans></header_block>"
        if tag == "CAPTION": return f"<caption_block><src>{txt}</src><trans>{txt}</trans></caption_block>"
        return match.group(0)
    content = re.sub(r'\[\[(HEADER|CAPTION):\s*(.*?)\]\]', replace_raw_tags, content)
    ...
    for match in block_pattern.finditer(content):
        ...
        if tag == "ref_block":
            ...
            txt = txt.replace('\n', ' ') 
            txt = re.sub(r'(\[\d+\])', r'<br><b style="color:#e74c3c;">\1</b>', txt)
        ...
        s_txt = src_m.group(1).strip() if src_m else inner
        t_txt = trans_m.group(1).strip() if trans_m else ""
        if not t_txt and tag == "header_block": t_txt = s_txt

        body += f"""<div class="row {css}"><div class="col src">{s_txt}</div><div class="col trans">{t_txt}</div></div>"""

    html = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>{display_title}</title>
    <style>
        ...
        h1 {{ background: var(--p); color: #fff; padding: 20px; margin: 0; text-align: center; word-wrap: break-word; }}
        .row {{ display: flex; border-bottom: 1px solid #eee; }}
        .col {{ flex: 1; padding: 20px; text-align: justify; min-width: 0; word-wrap: break-word; }}
    ...
    </style></head><body><div class="container"><h1>{display_title}</h1>{body}</div></body></html>"""
